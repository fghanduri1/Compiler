    ...
    public class TargetCode
    {
        ...
        public void PatchInstructionToJumpHere(Address address)
        {
            if (address < 0 || address >= Instructions.Count)
                Debugger.WriteDebuggingInfo("Tried to backpatch an instruction which is at an address that is not in the generated program");
            else
            {
                ...
                if (original.OpCode == OpCode.JUMP || original.OpCode == OpCode.JUMPIF)
                    ...
                    Debugger.WriteDebuggingInfo("Tried to backpatch an instruction which is not a JUMP or JUMPIF");
            }
        }
